...
| 스펙 → TDD 실행 계획 | `/writing-plans` | 문제 정의 완료 후, 응집된 work unit과 dependency wave 설계 |
| 계획 분산 실행 | `/subagent-driven` | 문제 정의 검증 → 적정 분해 → agent 수 산정 → wave 실행 |
...
| 요청 유형 | 스킬 | 진입 조건 |
|---|---|---|
| 동료 협업 톤, 반박/트레이드오프 | `/collaboration` | 제안·리뷰·반론 시, STYLE_GUIDE 동반 |
| 정량 예측 (QPS/row/p99) | `/metrics` | 설계·리뷰 pre-condition, 수치 누락 시 |
| 런타임 관측 (로깅/모니터링/Slack) | `/observability` | 대상 프로젝트 로깅·알림 설계 시 |
